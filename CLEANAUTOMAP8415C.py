...
#22:21 28.01.2026 SilverMiner clean automap from redundant lines
...
texmap72 = {'-': '-', 'LOOSERCK': 'DBNATU02', 'CTYSTCI4': 'DBBROW01', 'DRIPWALL': 'SUPPORT3\
', 'LAVA1': 'ASHWALL2', 'BRWNRCKS': 'ASHWALL3', 'LAVAFL1': 'LAVFALL1', 'DOORSTON\
': 'SPCDOOR4', 'CTYSTCI1': 'BRICK18A', 'DOORWOOD': 'BIGDOOR5', 'GATMETL4': 'MIDB\
ARS4', 'CSTLRCK': 'BRICK21', 'SKY3': 'ZIMMER1', 'WATRWAL1': 'WFALL1', 'DOOREXIT'\
: 'BIGDOOR6', 'GATMETL5': 'DBMIDT02'}

flatmap72 = {'FLAT516': 'F_0008', 'FLTWAWA1': 'FWATER1', 'FLOOR12': 'CEIL5_2', 'FLAT510': 'R\
ROCK17', 'FLAT509': 'F_0010', 'FLATHUH1': 'LAVA1', 'FLOOR30': 'CRATOP3', 'FLTLAV\
A1': 'SLIME09', 'FLAT506': 'FLAT10', 'FLAT517': 'GRASS2', 'F_SKY1': 'F_SKY1', 'F\
LOOR03': 'DEM1_7', 'FLOOR26': 'GATE4'}
def reversedict(a):
	res = {v: k for k, v in a.items()}
	return res
t72r,f72r = reversedict(texmap72),reversedict(flatmap72)
...
def retexback72(map):
	...
	for sd in a.sidedefs:
		sd.tx_up = t72r.get(sd.tx_up, sd.tx_up)
		sd.tx_low = t72r.get(sd.tx_low, sd.tx_low)
		sd.tx_mid = t72r.get(sd.tx_mid, sd.tx_mid)
	for s in a.sectors:
		s.tx_floor = f72r.get(s.tx_floor,s.tx_floor)
		s.tx_ceil = f72r.get(s.tx_ceil,s.tx_ceil)
	return a.to_lumps()

def cleanautomap(map):
        a = MapEditor(map)
        sdarr = a.sidedefs
        
        for ld in a.linedefs:
                if ld.back == Linedef.NONE or ld.front == Linedef.NONE:
                        continue
                backsec = a.sectors[sdarr[ld.back].sector]
                frontsec = a.sectors[sdarr[ld.front].sector]
                if backsec.z_floor != frontsec.z_floor or (
                        backsec.z_floor == backsec.z_ceil or (
                                frontsec.z_floor == frontsec.z_ceil
                                )
                        ):
                        continue
                ld.invisible = 1
...
def main(args):
	...
	if (len(args) < 2):
		...
		for name in inwad.maps.find(pattern):
			...
			#outwad.maps[name] = retexback72(inwad.maps[name])
			outwad.maps[name] = cleanautomap(inwad.maps[name])
